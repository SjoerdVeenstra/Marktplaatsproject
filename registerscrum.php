...
if($_SERVER["REQUEST_METHOD"] == "POST"){
    if(empty(trim($_POST["EMAIL"]))){
        ...
        $sql = "SELECT id FROM customers + 61?*/ WHERE email = ?";
    ...
    if(empty($email_err) && empty($password_err) && empty($confirm_password_err)){
        ...
        if($stmt = mysqli_prepare($link, $sql)){
            ...
            if(mysqli_stmt_execute($stmt)){
                ...
                header("location: registerscrum.php");  // veranderen?
            }
            else{
         ...
         <section class="section-aa">
            ...
            <div class ="form-group <?php echo ((!empty($confirm_password_err)) ? 'has-error' : ''; ?>">
                ...
                <span class="help-block"><?php echo $confirm_password_err; ?></span>     
                     ...
                     <input type="submit" name="submit" value="Submit" id="submit-button" class="btn btn-primary">
                     <input type="reset" class="btn btn-default" value="reset">
                     <p>Already have an account? <a href="loginscrum.php">Login here</a>.</p>
                </form>
        </section>
